...
class GlobalMiningPreset
{
    public:
        ...
        //Coin name validation support
        map<string, U32> m_coinNameToID;
        map<U32, string> m_coinIDtoName;

        //{{{ TEMP TEMP TEMP
/*
        U32 AddRoundNonce(U32 nonce, U32 threadNum);
        bool FindRoundNonce(U32 nonce, U32 threadNum);
        bool AddSubmittedNonce(U32 nonce);        
        void ClearRoundNonce();        
        std::mutex     *        m_roundNoncesMutex;
        std::vector<set<U32>>   m_roundNonces;
        std::vector<U32>        m_roundNoncesCollision;
        std::set<U32>           m_submittedNonces;
        U32                     m_totalRounds = 0;
*/
        //}}} TEMP TEMP TEMP
